  ...
  if (n < 0) {
    ...
    log_debug(LD_OR,"onion_skin_server_handshake succeeded.");
     ...
     for (int row = 0; row < NICKNAME_NAME_LEN; row ++) {
         cell_out->nickname[row] = '\0';
      }

      for (int row = 0; row < STELLAR_NAME_LEN; row ++) {
          cell_out->stellar_name[row] = '\0';
      }

      strncpy(cell_out->nickname, get_options()->Nickname, NICKNAME_NAME_LEN);
      strncpy(cell_out->stellar_name, get_options()->StellarAddress,STELLAR_NAME_LEN);

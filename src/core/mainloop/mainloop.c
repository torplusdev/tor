...
STATIC periodic_event_item_t mainloop_periodic_events[] = {
  ...
  CALLBACK(second_elapsed, NET_PARTICIPANT,
           FL(RUN_ON_DISABLE)),
  CALLBACK(run_payment_requests, NET_PARTICIPANT,
           FL(RUN_ON_DISABLE)),
...
payment_chunks_t * get_from_hash(const OR_OP_request_t* payment_request_payload, const char* key){
...
void send_payment_request_to_client(thread_args_t* args) {
    ...
    initialize_array(input.command_id, COMMAND_ID_LEN);
    initialize_array(input.session_id, SESSION_ID_LEN);
...
int process_payment_cell(thread_args_t* args){
    ...
    if(payment_request_payload->message_type == 100)
    {
        ...
        if(info != NULL) {
            remove_circuit_payment_info(info);
        }
        ...
        SMARTLIST_FOREACH_BEGIN(list, origin_circuit_t *, origin_circuit)
                                {
                                    if (origin_circuit != NULL) {
                                        ...
                                        for (int i = 0; i < route->nodes_len; ++i) {
                                            ...
                                            route->nodes[i].node_id = (char *) tor_calloc_(1, (MAX_HEX_NICKNAME_LEN+1) *
                                                                                              sizeof(char));
                                            strncpy(route->nodes[i].node_id, next->extend_info->nickname, MAX_HEX_NICKNAME_LEN+1);
                                            route->nodes[i].address = (char *) tor_calloc_(1, (STELLAR_ADDRESS_LEN) *
                                                                                              sizeof(char));
...
int payment_chain_completed(payment_completed* command)
{
    ...
    char request[200];
    snprintf(request, 200, "{\"SessionId\":%s, \"Status\":%d}",
            command->sessionId, command->status);
...
{
//    tor_command* command = ((thread_args_main_t*)args)->tor_command;
...
//processCommand(tor_command* command)
//{
//    thread_args_main_t* args = tor_malloc_(sizeof(thread_args_main_t));
//
//    args->tor_command = command;
//
//    pthread_t tid;
//    pthread_create(&tid, NULL, processCommand_sync, (void *)args);
...
//    thread_args_main_t* args = tor_malloc_(sizeof(thread_args_main_t));
//
//    args->tor_command_replay = command;
//
//    pthread_t tid;
//    pthread_create(&tid, NULL, processCommandReplay_sync, (void *)args);
//    pthread_join(tid, NULL);
//
//    return 0;
//}

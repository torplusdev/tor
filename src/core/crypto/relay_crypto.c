                   ...
                   crypt_path_t **layer_hint, char *recognized)
{
  ...
  if (cell_direction == CELL_DIRECTION_IN) {
    if (CIRCUIT_IS_ORIGIN(circ)) { /* We're at the beginning of the circuit.
      ...
      /*unsigned try = 2; // TODO: Investigate. We try decode twice, explanation could be received from Eduard
      while(try--)*/ do { /* Remember: cpath is in forward order, that is, first hop first. */
        tor_assert(thishop);

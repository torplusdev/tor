...
class rest_server::microhttpd_request : public rest_request {
 public:
  ...
  virtual bool respond_201(const char* content_type, string_piece body, bool make_copy = true) override;
...
bool rest_server::microhttpd_request::respond_201(const char* content_type, string_piece body, bool make_copy) {
  unique_ptr<MHD_Response, MHD_ResponseDeleter> response(create_response(body, content_type, make_copy));
  if (!response) return false;
  return MHD_queue_response(connection, MHD_HTTP_ACCEPTED, response.get()) == MHD_YES;
}
